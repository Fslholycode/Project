...
class Sampler:
    ...
    def sample_sentence(self, prefix, max_length = 100):
        ...
        i = 0
        sent = prefix
        previous = ["START_OF_SENTENCE", "START_OF_SENTENCE"]
        word = self.sample_next(previous, False)
        while i <= max_length and word != "END_OF_SENTENCE":
            sent.append(word)
            previous[0] = previous[1]
            previous[1] = word
            word = self.sample_next(previous)
            i += 1
        return sent

    def sample_next(self, previous, incl_eos = True):
        ...
        for w in self.lm.vocab():
            if not incl_eos and w == "END_OF_SENTENCE":
                continue
            lp = self.lm.cond_logprob(previous[0], previous[1], w)
